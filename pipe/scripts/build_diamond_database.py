...
def main(outdir, threads):
    with local.cwd(outdir):
        ...
        shell(
            "ls */*.fasta.gz | grep -v 'DNA' | grep -v 'additional' | "
        ...
        shell(
            "cat uniprot_ref_proteomes.fasta | sed -r "
        ...
        shell(
            "diamond makedb --in uniprot_ref_proteomes.fasta -d "
